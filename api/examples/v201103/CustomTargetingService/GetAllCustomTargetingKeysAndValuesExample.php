...
try {
  ...
  $customTargetingService = $user->GetCustomTargetingService('v201103');
  ...
  $keyCount = 0;
  foreach ($keys as $key) {
    printf("%d) Custom targeting key with ID '%.0f', name '%s', display " .
    ...
    if (array_key_exists($key->id, $keyIdToValueMap)) {
      ...
      foreach ($keyIdToValueMap[$key->id] as $value) {
        printf("\t%d) Custom targeting value with ID '%.0f', name '%s', and " .

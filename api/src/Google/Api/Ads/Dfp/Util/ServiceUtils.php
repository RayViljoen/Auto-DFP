...
class ServiceUtils {
   ...
   * @var string regular expression to determine if method name is
   *     get*ByStatement
   * @access private
   */
  private static $GET_BY_STATEMENT_METHOD_NAME_REGEX = '/^get.*ByStatement$/';

  /**
   * The regular expression used to determine if the filter text is
   * incompatible.
   * @var string regular expression used to determine if the filter text is
   *     incompatible.
   * @access private
   */
  private static $INCOMPATIBLE_FILTER_TEXT_REGEX =
      '/(?i)(?<=^|\s)(LIMIT|OFFSET)(?=$|\s)/';
  ...
  public static function GetAllObjects(DfpSoapClient $service,
    ...
    if (!preg_match(ServiceUtils::$GET_BY_STATEMENT_METHOD_NAME_REGEX,
        $methodName)) {
      throw new InvalidArgumentException(
          'The method name must be in the format a "get*ByStatement".');
    }
    $matches = array();
    if (preg_match(ServiceUtils::$INCOMPATIBLE_FILTER_TEXT_REGEX, $filterText,
        $matches)) {
      throw new InvalidArgumentException(
    ...
    do {
      $filterStatement->query = $filterText . ' LIMIT '
          . ServiceUtils::$PAGE_SIZE . ' OFFSET ' . $offset;
      $page = $service->$methodName($filterStatement);
      if (isset($page->results)) {
        $allObjects = array_merge($allObjects, $page->results);
      }
      $offset += ServiceUtils::$PAGE_SIZE;
    } while (isset($page->results)
          && sizeof($page->results) == ServiceUtils::$PAGE_SIZE);
